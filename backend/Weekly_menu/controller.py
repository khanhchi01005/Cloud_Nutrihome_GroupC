...
from Weekly_menu.services import get_weekly_menu_service_2, get_daily_nutrition_service, upload_receipt_service, check_eaten
# Định nghĩa Blueprint cho các route liên quan đến weekly menu
menu_bp = Blueprint('menu', __name__)

# Route cho API lấy danh sách thực đơn hàng tuần
@menu_bp.route('/api/weekly_menu_2', methods=['GET'])
def get_weekly_menu_2():
    # Lấy user_id từ query param, ví dụ /api/weekly_menu_2?user_id=1
    data = request.json 
    ...
    result, status_code = get_weekly_menu_service_2(user_id)
